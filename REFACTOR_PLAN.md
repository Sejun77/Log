...
**9-C — Remove Tier 3 from `resolvedTemplates` + resume fallback (split per 2026-05-21 planning audit into 9-C1 orphan fallback + 9-C2 `resolvedTemplates` strip; 9-C1 shipped 2026-05-22, 9-C2 still pending):**
...
**9-C2 — Strip Tier 3 from both `resolvedTemplates` variants (gated on diagnostic):**

- [ ] Strip the Tier 3 arm from `Entities.swift:547 RoutineExercise.resolvedTemplates()` and `RoutineExercise+Helpers.swift:43 resolvedTemplates(in: ctx)`. Both functions return `[]` for the formerly-Tier-3 case. The active-workout UI's `SessionPlanResolver.effectiveSetCount` clamp at `max(1, …)` keeps at least one row rendering; `plannedRepTarget` / `plannedDurationTarget` source from sessionPlan/snapshot which are populated for any started workout, so the empty `targetReps: 0` / `targetWeight: nil` template values are never reached
- [ ] **Note:** the `(in: ctx)` variant's Tier-3 arm also carries a hidden side effect — it normalizes `ex.defaultTemplates[i].order` via `normalizeOrderIfNeeded` and persists the fix. This side effect goes away with the Tier-3 strip. Acceptable because `ExercisesView.normalizeTemplateOrderIfNeeded` (9-D scope) already self-heals the same state when the user opens the Exercise editor
- [ ] **Update `SlotPrescriptionResolutionTests`** (added in 9-pre tests):
  - **Invert + rename** to `…ReturnsEmptyEvenWhenDefaultsPresent`: `testResolvedInCtx_Tier3UsedWhenSetTemplatesEmptyAndPrescriptionNil`, `testResolvedInCtx_Tier3UsedWhenPrescriptionPresentButHasNoContent`, `testResolvedNoCtx_Tier3WhenSetTemplatesEmptyAndPrescriptionNil`, `testResolvedNoCtx_Tier3WhenPrescriptionPresentButHasNoContent`
  - **Delete**: `testResolvedInCtx_Tier3PreservesOrder` (no Tier 3 to test order on)
  - **Keep unchanged**: `testResolvedInCtx_AllTiersEmptyReturnsEmpty`, all `testHasContent*` / `testGenerateTemplates*` / Tier 1 / Tier 2 cases, `testExerciseDefaultTemplatesWorkingCountReflectsKindFilter` (orthogonal to Tier 3)
  - **Add**: `testResolvedInCtx_DoesNotReadExerciseDefaultTemplates` + mirror for no-context variant (fixture with sentinel `[working(reps: 99, weight: 999)]` in defaults; assert returned `[]` never contains the sentinels)
  - **Add**: `testResolvedInCtx_NormalizationSideEffectIsLimitedToTier1` — pin that the `(in: ctx)` variant no longer normalizes `ex.defaultTemplates[i].order` (snapshot orders before, invoke resolver, assert unchanged)
- [ ] **Rewrite** `HydrateEmptySlotPrescriptionsTests.testGoldenBehaviorPreservation`: currently asserts `resolvedTemplates(in:)` pre-backfill (Tier 3) matches post-backfill (Tier 2). Post-9-C2 the pre-backfill call returns `[]`, breaking the assertion. Rewrite as "post-backfill `resolvedTemplates(in:)` produces the expected Tier 2 output for a Class-B slot" — golden behavior lives at the Tier 2 generation level only after 9-C2
- [ ] **Gating (per 9-A.5 audit + 2026-05-21 9-C planning audit):**
  - 9-A must have shipped on TestFlight for at least one release so the backfill has hydrated real-user legacy slots before this slice strands them
  - 9-B (both 9-B1 and 9-B2) must have shipped so no non-Tier-3 read site still depends on `Exercise.defaultTemplates`
  - 9-C1 must have shipped so the orphan-fallback no longer reads `defaultTemplates` either
  - 9-pre diagnostic must have shipped on TestFlight and reported `residualEmptyContentSlots == 0 AND slotsNeedingTier3 == 0` for at least one release on real-user data. If either counter is non-zero, **block 9-C2 until the hydration gap is fixed**
- [ ] **Optional integration test**: pin that `BackfillService.diagnoseDefaultTemplatesRisk(in:).slotsNeedingTier3 == 0` after running hydration on a representative legacy fixture. Permanent regression net for the post-9-C2 "no slot stranded" invariant
- [ ] **"Unprogrammed slot" routine-editor UX (deferred — optional)**: was originally bundled into 9-C; the 2026-05-21 audit moves it out because the post-9-A2 population that would surface it should be statistically zero. Ship as a polish slice (9-C.5 or fold into 9-D) only if the diagnostic ever shows non-zero `slotsOrphanedNoSource`
...
Phase 9 is **not closed** — 9-C2 / 9-D / 9-E are still pending (Phase 9-C is partially complete: 9-C1 shipped, 9-C2 pending). Shipped so far: pre-9 cleanup (8-C), 9-pre regression tests, **9-A** in full (9-A1 hydration service + 9-A2 bootstrap wiring), **9-A.5 audit decisions** (planning), **9-B** in full (9-B1 routine-editor reads + 9-B2 ActiveWorkout swap path incl. follow-up swap/resume/timer fixes), the **9-pre / pre-9-E `defaultTemplates` risk diagnostic** (DEBUG-only launch logging + test-callable helper), **9-C planning audit** (split 9-C into 9-C1 + 9-C2), and **9-C1** (`WorkoutResumeService` orphan-fallback rewrite — `defaultTemplates` read removed from `Log/Services/WorkoutResumeService.swift`; orphan-resume now uses `makeSwapDefaultTemplates(...)` at AppSettings defaults). Sub-slices are independently revertible. Recommended ship spacing: **9-pre tests** ✅ → **9-pre diagnostic** ✅ → **9-A1** ✅ → **9-A2** ✅ → **9-A.5 audit decisions** ✅ → **9-B1** ✅ → **9-B2** ✅ → **9-C1** ✅ (orphan-fallback rewrite shipped 2026-05-22) → release gap + observe diagnostic on TestFlight → **9-C2** (`resolvedTemplates` Tier 3 strip — gated on `residualEmptyContentSlots == 0 AND slotsNeedingTier3 == 0`) → **9-D** → **9-E pre-flight migration decision** (gated on `defaultTemplatesWithTargetWeight` + `defaultTemplatesNonWorkingKind`) → **9-E field deletion**.
