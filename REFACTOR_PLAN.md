...
**Phase 11 closure note**: with 11.6-A and 11.6-B both shipped, the **file-decomposition objective of Phase 11 is functionally complete** — every previously-identified zero-or-low-risk extraction has landed (top-level support types, Routines decomposition × 3 passes, ActiveWorkout supporting structs, RoutineEditor, ActiveWorkout pure utilities, `SessionPlanResolver` service with 23 tests). What remains in `ActiveWorkoutView.swift` is dominated by the `body` + the six `@ViewBuilder` methods (~1,500 LOC combined; Phase-12 territory) plus struct properties and the per-concern helpers that 11.6-C would relocate. **Phase 11 nonetheless remains formally open** because 11.6-C and 11.6-D are still `Pending` — neither has shipped nor been explicitly deferred to Phase 12. Closing Phase 11 requires an owner decision: either (a) ship 11.6-C (medium risk — forces the `@State` surface widening) ± 11.6-D, or (b) explicitly defer 11.6-C/D to Phase 12 (alongside the already-deferred `@ViewBuilder` methods, since the bulk of the remaining ~3,030 LOC is Phase-12 work regardless of whether 11.6-C ships).
...
† Post-11.6-A + 11.6-B intermediate: **`ActiveWorkoutView.swift` 3,061 → 3,030 LOC (−31, ≈0%)**. 11.6-A moved 4 helpers out (−34 LOC, 3,061 → 3,027); 11.6-B shipped the `SessionPlanResolver` service with thin wrappers retained on the view (+3 LOC, 3,027 → 3,030 — the audit's ~−80 estimate assumed direct call-site rewrites that were deliberately deferred). The "After 11.6" column shows the projected floor only if 11.6-C ships in full; otherwise the realistic floor with just A+B is **~3,030 LOC**, since the body + `@ViewBuilder` methods (Phase-12 territory) dominate what's left.
