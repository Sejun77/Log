...
- [ ] Expected `RoutinesView.swift` reduction: 913 → ~430 (post-11.3 baseline). `RoutinesView` itself remains in place as the top-level list (`@Query` over `Routine`, the routine-list row UI, `Add Routine` / delete / lock / order-backfill flows) plus the file-private `BlockRow` + `LockBadge` deferred from 11.3 (see "Deferred badge cleanup" below)
...
- Tracker: ~40 LOC will move out of `RoutinesView.swift` once unblocked. Post-11.5 `RoutinesView.swift` estimate (~430 LOC) already excludes this delta, so unblocking only matters for cosmetic consolidation of the badge variants
...
| `RoutinesView.swift` | 2,543 | 2,543 | **1,896** | **913** | 913 | ~430 | ~430 |
