...
**Pending (11.2 — Routines pass 1: pickers + warmup editor + prescription fields):**

- [ ] Extract `ExercisePickerSingle` + `SupersetPicker` (~140 LOC) to `Log/Main/Routines/ExercisePickers.swift`
- [ ] Extract `WarmupSchemeEditor` + `WarmupStepRow` + `WarmupStepEditSheet` (~225 LOC) to `Log/Main/Routines/WarmupSchemeEditor.swift`
- [ ] Extract `SlotPrescriptionSection` + `PrescriptionFields` + `TempoEditorView` + the `makeDefaultPrescription` factory (~280 LOC after co-location) to `Log/Main/Routines/PrescriptionFields.swift`. `makeDefaultPrescription` must bump from `fileprivate` to default-internal so `RoutineEditor.appendBlock` (still in `RoutinesView.swift` until 11.5) and `SupersetDetailNoRest.addExercise` (will move in 11.3) can continue to find it. **The only access change in this slice.**
- [ ] Expected `RoutinesView.swift` reduction: 2,543 → ~1,940 (−~600 LOC)
- [ ] **Lowest-risk Routines slice** — all moved types take `@Bindable` model values; no shared `@State`, no cross-type coupling beyond `makeDefaultPrescription`
- [ ] Manual smoke: open routine editor → Add Exercise → Add Superset → edit prescription fields on a normal block → open warmup editor → add/edit/reorder/delete steps → re-open superset and verify per-exercise `SlotPrescriptionSection` rows still render with `hideSetsField` + `hideRestFields`

**Pending (11.3 — Routines pass 2: technique editor + block detail views + block row + model helpers):**

- [ ] Extract `TechniquePlanEditor` + `TechniquePlanRow` + `TechniqueTypePickerSheet` + `TechniqueParamEditView` (~588 LOC — biggest single move in Phase 11) to `Log/Main/Routines/TechniquePlanEditor.swift`
- [ ] Extract `RoutineBlockDetailView` + `SupersetDetailNoRest` (~336 LOC) to `Log/Main/Routines/BlockDetailViews.swift`
- [ ] Extract `BlockRow` + `LockBadge` (~40 LOC) to `Log/Main/Routines/BlockRow.swift`
- [ ] Extract `extension RoutineExercise { safeExercise(in:), normalizeOrderIfNeeded(_:) }` (~65 LOC) to `Log/Models/RoutineExercise+Helpers.swift`
- [ ] Expected `RoutinesView.swift` reduction: ~1,940 → ~970 (−~970 LOC; ~60% cumulative reduction from pre-Phase-11 baseline)
- [ ] Manual smoke: add a technique to a slot → pick technique type → edit params for dropset (`dropCount` / `dropPercent` / `restSeconds` / `dropsetEffort`) and cluster (`rounds` / `reps` / `restSeconds`) → verify roundtrip; confirm locked-routine path (lock badge + scrollable-but-disabled detail views) still works; rename / delete an exercise and verify `safeExercise(in:)` fallback to "Deleted exercise" in routine + history rows

**Pending (11.4 — ActiveWorkout supporting private structs):**

- [ ] Move the 9 private subview structs currently at `ActiveWorkoutView.swift:3204–3787` into a new `Log/Main/ActiveWorkout/` directory:
  - `SetRows.swift` — `SetEntryRow` + `TimeSetEntryRow` (~150 LOC)
  - `DropLogRow.swift` — `DropLogRow` (~85 LOC)
  - `TechniqueChipsViews.swift` — `TechniqueIndicatorRow` + `SetTechniqueChipsRow` + `TechniqueDetailSheet` (~155 LOC)
  - `RestOverlayScreen.swift` — `RestOverlayScreen` (~50 LOC)
  - `ExerciseNotesEditSheet.swift` — `ExerciseNotesEditSheet` (~60 LOC)
  - `EditSessionPlanSheet.swift` — `EditSessionPlanSheet` + its three `stepperRow` / `optionalString` private helpers (~140 LOC)
- [ ] Each struct is **already presented to `ActiveWorkoutView` via plain initializers** (Bindings, value types, or callback closures); none captures `ActiveWorkoutView` instance state. Low-risk pure-move
- [ ] All moved structs bump from `private struct` to `struct` (default-internal) — same access pattern as 11.2 / 11.3
- [ ] Expected `ActiveWorkoutView.swift` reduction: ~3,695 → ~3,100 (−~600 LOC)
- [ ] Manual smoke: log reps / weight set; log time-based set; log a drop sub-log; tap a technique chip → detail sheet opens; tap "Edit Exercise Notes" → notes sheet opens, Done saves, Cancel reverts; tap "Edit Plan" → session-plan editor opens and Save / Cancel both work; open the rest overlay → close button + countdown work

**Pending (11.5 — RoutineEditor extraction):**

- [ ] Extract `struct RoutineEditor: View` (~536 LOC) from `RoutinesView.swift` to `Log/Main/Routines/RoutineEditor.swift`. The largest remaining `RoutinesView.swift` consumer of `RoutineExercise+Helpers` / `PrescriptionFields` / `BlockDetailViews` / `ExercisePickers` (all relocated in 11.2 / 11.3); the extraction completes the Routines decomposition
- [ ] Expected `RoutinesView.swift` reduction: ~970 → ~430. `RoutinesView` itself remains in place as the top-level list (`@Query` over `Routine`, the routine-list row UI, `Add Routine` / delete / lock / order-backfill flows)
- [ ] **Medium-risk** — `RoutineEditor` carries its own `@Query` for `allExercises` (already module-internal) and several private helpers. The extraction should preserve the helpers as `private` inside the new file
- [ ] Manual smoke: full routine-editor flow — add block, edit block, add superset, edit superset details, reorder blocks, delete a block, swap an exercise, lock a routine while a workout is active and verify edit mutation surfaces remain disabled

**Pending (11.6 — ActiveWorkoutView per-concern extension files, lowest priority):**

- [ ] Split the ~60 private methods on `ActiveWorkoutView` into per-concern extension files under `Log/Main/ActiveWorkout/`:
  - `ActiveWorkoutView+Persistence.swift` — `persistRestState`, `clearPersistedRestState`, `persistSessionPlans`, `restoreSessionPlansFromAppState`, `persistPosition`, `restorePositionFromAppState`, `restoreStableRestID`, `startRestWithPersistence`, `resumeRestFromAppState`, `restoreDropWeightDrafts` (~10 helpers)
  - `ActiveWorkoutView+Swap.swift` — `swapExercise(planExercise:with:)`, `performPendingSwap(resetPlan:)`, `applyExerciseSwapsToRoutine` (~3 helpers, ~200 LOC)
  - `ActiveWorkoutView+Snapshot.swift` — `populateSnapshotFields(on:from:)`, `rebuildItemsByExerciseID`, `ensureInputsInitializedFromPlan`, `rehydrateFromWorkoutIfPresent` (~4 helpers)
  - `ActiveWorkoutView+SupersetHelpers.swift` — `lastRoundIndex`, `supersetRoundComplete`, `allExercisesLogged`, `advanceForSupersetAfterLog`, `isAtLast` (~5 helpers)
  - `ActiveWorkoutView+TechniqueHelpers.swift` — `techniquesApplying`, `buildTechniqueChips`, `dropsetTechniqueApplying`, `suggestedDropWeight`, `roundWeight`, `formatWeight`, `appendDropLog`, `undoDropLog` (~8 helpers)
- [ ] Behavior-preserving — methods stay on `ActiveWorkoutView` via Swift `extension ActiveWorkoutView { … }`, just relocated. Module-internal access by default; nothing changes
- [ ] **Lowest priority in Phase 11.** Adds 5–8 glue files of mostly-method-relocations with modest readability gain — the body of the view itself is the limiting factor for further size reduction, and that's a Phase 12 concern (viewmodel hoist), not a file-split concern
- [ ] Expected `ActiveWorkoutView.swift` reduction: ~3,100 (post-11.4) → ~2,000–2,300. Concrete reduction depends on how the extension files are split and whether any tiny helpers stay co-located with `body`

**Deferred to Phase 12 (MVVM / viewmodel) — NOT Phase 11 file decomposition:**

- `ActiveWorkoutView`'s `@ViewBuilder` methods that capture `@State`: `buildSetRow(block:exercise:idx:template:)`, `buildWarmupRow(block:exercise:step:)`, `buildDropSection(block:exercise:parentSetIndex:)`, `buildWorkingSetGroup(block:exercise:idx:template:)`. These methods read `inputsByExerciseID`, `loggedByExercise`, `dropsLoggedByExercise`, `parentDraftStore`, `dropWeightDraftStore`, `currentExerciseIndex`, `currentBlockIndex`, plus session-plan, draft, and rest-firing side effects. Splitting them across files without breaking the SwiftUI `@State` capture model requires either hoisting state into an `ObservableObject` viewmodel or passing dozens of `Binding`s / closures — that's a logic refactor, not a file split. **Defer until Phase 12**
- `restSecondsAfterCurrentLog(setIndex:template:block:exercise:)` — post-Phase-7.4-C, this method is largely a shell over `RestPlanner` calls plus a few inline guards. Moving it to an extension file is mechanical (it doesn't capture `@State` beyond `self`) and could happen as part of 11.6 if helpful. **Optional inclusion in 11.6**

**Phase-11 progress snapshot:**

| Target file | Pre-11.1 | Post-11.1 | After 11.2 | After 11.3 | After 11.4 | After 11.5 | After 11.6 |
|---|---:|---:|---:|---:|---:|---:|---:|
| `ActiveWorkoutView.swift` | 3,849 | **3,695** | 3,695 | 3,695 | ~3,100 | ~3,100 | ~2,000–2,300 |
| `RoutinesView.swift` | 2,543 | 2,543 | ~1,940 | ~970 | ~970 | ~430 | ~430 |
