...
**Pending (broader Phase 7 — original coverage gaps; partially covered by 7.6):**
...
- [ ] Test: finishing a workout produces immutable history and clears active state — *partially covered*: `AppStateLifecycleTests` pins the active-state clear contract at the model layer; the SwiftUI button closure that triggers it (`finishWorkout(applySwaps:applySlotPrescription:)` in `ActiveWorkoutView`) remains view-coupled and unit-untestable until a `WorkoutLifecycleService` extraction (see new optional pending item below)
- [ ] Test: **Save & Exit** preserves the workout as resumable (`Workout.completedAt == nil`, `AppState.workoutState == .active`, `AppState.activeWorkoutID == workout.id` all remain set after dismiss); **Discard** deletes the workout and clears active state — *partially covered*: `WorkoutResumeServiceTests` 2–7 + `AppStateLifecycleTests` 2 + 5 pin the **service/model contract** that resume relies on, but the actual Save & Exit / Discard button closures in `ActiveWorkoutView.swift:1448-1459` cannot be invoked from XCTest (inline `.confirmationDialog` closures capturing `workout`, `ctx`, `dismiss`). Full closure-level coverage requires either UI tests or a `WorkoutLifecycleService` extraction (see optional pending item below). **Note**: this item's original wording said *"preserves workout with `completedAt` set"* — that described the pre-Phase-8-A buggy behavior. The corrected post-fix semantics are now reflected here
...
- [ ] **Optional (Phase 7.7 or Phase 8-B follow-up): extract `WorkoutLifecycleService`** lifting the SwiftData-level effects of Save & Exit / Finish / Discard out of `ActiveWorkoutView`'s inline button closures into a pure `@MainActor enum` namespace taking `(workoutID:in:)`. Would enable a 3rd test file `LogTests/WorkoutLifecycleServiceTests.swift` with ~6 cases pinning `completedAt` is/isn't set + `AppState.workoutState` / `activeWorkoutID` is/isn't cleared per branch. Closes the last gap left by 7.6. **Not bundled into 7.6** because the refactor touches production view code (medium risk vs. 7.6's zero-production-change scope) — belongs in its own slice
...
Phase 7 is **not closed** — broader coverage gaps + performance items above remain. 7.6 only ships the lifecycle/resume service-and-model contract coverage.
