...
Last updated: 2026-05-18 (KST) — Phase 7 Slices 7.0 + 7.1 + 7.2 + 7.3 + 7.4 (RestTimer + ParentDraftStore sub-slices) + 7.5 complete: `LogTests` XCTest target wired with an in-memory `ModelContainer` harness and full suite at 51/51; the 7.4 ParentDraftStore sub-slice (7.4-A) lifted parent working-set draft persistence out of `ActiveWorkoutView.swift` into `Log/Services/ParentDraftStore.swift` (storage layout byte-identical so in-flight drafts survive the refactor) and added 13 pure-XCTest cases including a literal-string format pin. The 7.4 RestTimer sub-slice had already added 7 cases. Drop-weight draft extraction (7.4-B) and the rest-decision extraction (7.4-C) remain pending. Slice 7.3 extracted Phase 6.B backfill into `BackfillService`; Slice 7.2 extracted `RoutineLabelResolver`; Slice 7.5 aligned the test target's deployment to 18.5 and documented the concrete-simulator + app-hosted policy in CLAUDE.md. Host-less conversion attempted and reverted (iOS app targets can't link `@testable` symbols without `TEST_HOST` / `BUNDLE_LOADER`). Phase 6.B Slice C.1 / Slice B remain shipped; Slice C.2 grouping and rename verification still pending
...
**Pending (7.4-B — `DropWeightDraftStore` extraction + tests):**

- [ ] Sibling extraction for the drop-weight draft system in `ActiveWorkoutView` (≈L2378–2405 + the `restoreDropWeightDrafts` bridge at L2466). Same `UserDefaults`-backed shape as `ParentDraftStore` but with already-stringly-typed slot keys (`"<slotID>_<parentSetIndex>_<subIndex>"`). The `restoreDropWeightDrafts` helper that mutates `@State dropWeightInput` / `dropWeightUserEdited` cannot move wholesale — keep that bridge inside the view and have it call `store.allEntries() -> [String: String]`. Preserve storage layout `"dropWeightDrafts_<workoutUUID>"` byte-for-byte. Tests mirror the `ParentDraftStore` suite shape

**Pending (7.4-C — rest-decision extraction):**
