...
**Pending (7.4-C.4 — non-final intra-drop helper + inline-dropset variants, if still desired):**

- [ ] **Non-final intra-drop rest** (the `else` branch of `isFinalDrop` in `buildDropSection.onLog`): single-line read of `snap.restSeconds.flatMap { $0 > 0 ? $0 : nil }`. Extraction would expose `RestPlanner.restSecondsAfterIntraDrop(_:)` taking an `Int?` and returning the same `> 0` filter. Tiny payoff vs. one new function name; document the no-extract decision unless a future caller benefits
- [ ] **`dropsetTechniqueApplying != nil` technique-suppression branch** in `restSecondsAfterCurrentLog` (returns `nil` to defer rest to the final sub-log): a one-line guard `restSec = nil`. Same pay-off / API-surface tradeoff as the intra-drop case
- [ ] **Template-kind `.dropset` branch** in `restSecondsAfterCurrentLog` (the legacy template-based dropset path, separate from the technique-based dropset path that 7.4-C.3 covered): uses `plannedRestBetweenSets(for:) ?? priorWorkingRest(in: exercise.templates, upTo: idx)` with the `> 0` filter. Co-extracting requires moving the local `priorWorkingRest(in:upTo:)` helper out of `restSecondsAfterCurrentLog` and into a pure planner helper. Most users today use technique-based dropsets, so this branch fires rarely — low priority but a natural completion of the C-series extractions
- [ ] **Decide-or-close**: if pursued, extract all three above + add focused tests. Otherwise close out 7.4-C.4 with a "kept inline, not worth extracting — boolean guards / single call sites / legacy-only" note. Lowest-priority sub-slice in Phase 7
