...
**Status (2026-05-21): planning audits complete; 9-pre regression-pinning tests shipped; 9-A1 hydration service + tests shipped; 9-A2 bootstrap wiring still pending.** Two audits on 2026-05-20 mapped every read/write of `Exercise.defaultTemplates` and designed the legacy-slot backfill. On 2026-05-21 the 9-pre regression-pinning tests landed (`SlotPrescriptionResolutionTests`, 25 cases, +0 production LOC) — they freeze the current three-tier fallback so the upcoming backfill and 9-C Tier-3 removal cannot silently change resolution behavior. A **second 9-A audit pass on 2026-05-21** refined the exact `SetTemplate → SlotPrescription` field mapping, recorded which fields cannot be mined (autoreg, `targetWeight`, warmup, techniques), and surfaced a new **9-A.5 pre-9-C audit** (see below). Later the same day **9-A1** shipped (`BackfillService.hydrateEmptySlotPrescriptions(in:)` + `HydrateEmptySlotPrescriptionsTests`, 16 cases, ~+180 LOC service / ~+450 LOC tests; full suite 200/200) — the service is callable but **not yet wired into `BootstrapRoot`** (that's 9-A2, one-line production change). `Exercise.defaultTemplates` and both `resolvedTemplates` Tier-3 arms remain **load-bearing and untouched**. Headline findings:
...
- [ ] **9-pre diagnostic (optional, TestFlight only):** One-shot `os.Logger` count at `BootstrapRoot.body.task` of (a) `RoutineExercise` rows with `prescription == nil` post-`backfillPhase3_1`, (b) rows with `prescription?.hasContent == false`, (c) rows whose only template source is `Exercise.defaultTemplates`. No UI, no AppState field, no production behavior change. Ship + observe in TestFlight before 9-A backfill begins so the success criterion ("residual empty-content slots → 0") becomes measurable on real-user data; remove the logger in the same PR as 9-A

**9-A — Backfill empty `SlotPrescription` content (split 9-A1 shipped / 9-A2 pending wiring):**
...
- [ ] Optional: small `BootstrapRoot` integration test pinning the call order (or document the order in `BootstrapRoot`'s doc-comment if a test is over-investment for a one-line wiring)
- [ ] **Ship 9-A2 standalone.** Net diff: 1-line production change + optional integration test. Do NOT bundle the 9-pre diagnostic, reader changes, or UI removal. Tier 3 remains intact through 9-C, so a missed slot is still not a regression
...
- [ ] `ActiveWorkoutView.swapExercise` (`~L1908`): replace the `newEx.defaultTemplates.sorted...map` template-seed block with values from the swapped-in exercise's `RoutineExercise.prescription` (if a routine slot exists) or `AppSettings`-derived defaults
- [ ] `Routines/RoutineEditor.appendBlock` superset working-set-count gate (`L509-511`): replace `ex.defaultTemplates.filter { .working }.count` with `(slot.prescription?.sets ?? AppSettings.defaultSets)`. Since post-9-A every slot has `prescription?.sets != nil`, this is a clean swap
- [ ] `Routines/ExercisePickers.SupersetPicker.setCount(for:)` (`L62`): same migration. Note: this fires at picker time, before a `RoutineExercise` exists for the candidate, so falls back to `AppSettings.defaultSets` uniformly. Side-effect: makes every superset validate as "matching counts" — that's an authoring guardrail loss but not a correctness regression
- [ ] `Routines/RoutineEditor.normalizeRoutineModel` exact-copy detection (`L319-371`): post-9-A every slot's Tier 2 fires, so Tier-1-vs-Tier-3 exact-copy detection becomes vacuous; either remove the block or compare against `prescription.generateTemplates()` if the "clear redundant overrides" UX is worth keeping
- [ ] Add `LogTests/SupersetCountGateTests.swift` pinning the prescription-driven count behavior across appendBlock + picker paths
...
- [ ] **Requires 9-A to have shipped on TestFlight for at least one release** so the backfill has hydrated real-user legacy slots before this slice strands them. If the 9-pre diagnostic still shows residual empty-content slots, **block 9-C until the backfill is fixed**
...
Phase 9 is **not closed** — 9-A2 / 9-B / 9-C / 9-D / 9-E are still pending. Shipped so far: pre-9 cleanup (8-C), 9-pre regression tests, and 9-A1 (hydration service + tests; service not yet wired). Sub-slices are independently revertible. Recommended ship spacing: **9-pre tests** ✅ → **9-pre diagnostic** (TestFlight only, optional) → **9-A1** ✅ → **9-A2** (bootstrap wiring) → release gap + observe diagnostic → **9-A.5 audit decisions** (planning, blocks 9-C) → **9-B** → **9-C** → **9-D** → **9-E**.
