...
Last updated: 2026-05-18 (KST) — Phase 7 Slices 7.0 + 7.1 + 7.2 + 7.3 + 7.5 complete: `LogTests` XCTest target wired with an in-memory `ModelContainer` harness and full suite at 31/31; Slice 7.3 extracted the Phase 6.B launch-time backfill from `BootstrapRoot.swift` into `Log/Services/BackfillService.backfillRoutineVariantIDs(in:)` and added 12 tests covering routineID match, name fallback, no-overwrite, unresolved-stays-nil, idempotency, and preferred-variant delegation. Slice 7.2 had already extracted `RoutineLabelResolver` and Slice 7.5 aligned the test target's deployment to 18.5 + documented the concrete-simulator + app-hosted policy in CLAUDE.md. Host-less conversion attempted and reverted (iOS app targets can't link `@testable` symbols without `TEST_HOST` / `BUNDLE_LOADER`). Phase 6.B Slice C.1 / Slice B remain shipped; Slice C.2 grouping and rename verification still pending
