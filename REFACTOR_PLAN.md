...
**Pending (active-workout identity — duplicate `Exercise` across routine slots shares log / input state):**

- [ ] If the same `Exercise` (by `Exercise.id`) appears in two different `RoutineExercise` slots — either in different blocks of the same routine or twice in one block — logging in one slot incorrectly marks the other slot as logged, and input fields share state across slots. Likely root cause: active-workout state dictionaries are keyed by `Exercise.id` instead of the per-slot identity (`PlanExercise.id` / `RoutineExercise.id` / `routineSlotID`). Suspect keys to audit during the fix: `loggedByExercise[Exercise.id: Set<Int>]`, `dropsLoggedByExercise[Exercise.id: ...]`, `inputsByExerciseID[Exercise.id: ...]`, the `currentExercise` / focus-advance logic, the `parentDraftStore.load/persist/clear` slot ID parameter (today receives `exercise.id` which happens to equal `PlanExercise.id` but should be explicitly slot-scoped going forward), the `dropWeightDraftStore` slotKey format (`"<slotID>_<parentSetIndex>_<subIndex>"`), `isWorkingSetComplete(exercise:setIndex:)`, `supersetRoundComplete`, and all draft-restore paths. Fix requires deciding the canonical per-slot key (most likely `PlanExercise.id`, which is already a stable per-slot UUID) and migrating every `[Exercise.id: ...]` dictionary read/write. Out of scope for the RestPlanner extraction; the planner's `SupersetRoundParticipant.isComplete` is caller-provided so it will inherit whatever fix lands here. Track separately — broader than Phase 5.2's "rest semantics + superset flow streamline" scope but parked here until a more natural home (a future "active-workout integrity" sub-slice or a dedicated phase) is opened
