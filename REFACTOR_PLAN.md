...
Phase 9 is **not closed** — **9-E is the only remaining sub-slice** (Phase 9-C complete: 9-C1 + 9-C2 both shipped 2026-05-22; Phase 9-D complete: editor UI + all production app-side writes removed 2026-05-22, commit `5819fa6`). Shipped so far: pre-9 cleanup (8-C), 9-pre regression tests, **9-A** in full (9-A1 hydration service + 9-A2 bootstrap wiring), **9-A.5 audit decisions** (planning), **9-B** in full (9-B1 routine-editor reads + 9-B2 ActiveWorkout swap path incl. follow-up swap/resume/timer fixes), the **9-pre / pre-9-E `defaultTemplates` risk diagnostic** (DEBUG-only launch logging + test-callable helper), **9-C planning audit** (split 9-C into 9-C1 + 9-C2), **9-C** in full (**9-C1** `WorkoutResumeService` orphan-fallback rewrite — `defaultTemplates` read removed from `Log/Services/WorkoutResumeService.swift`; orphan-resume now uses `makeSwapDefaultTemplates(...)` at AppSettings defaults; and **9-C2** `resolvedTemplates` Tier 3 strip — both no-context `Entities.swift` and context-aware `RoutineExercise+Helpers.swift` resolvers are now strictly 2-tier (Tier 1 setTemplates → Tier 2 prescription → `[]`), commit `e9ba172`), and **9-D** Exercise-tab defaults editor UI removal (the entire `Section("Sets")` block + `SetTemplateRow` + 7 helpers + 4 `@State` vars stripped from `ExercisesView.ExerciseDetailView`; Time-based toggle now a direct `$exercise.isTimeBased` binding with no defaults-mutating side effects; **all production app-side writes to `Exercise.defaultTemplates` are now removed** — the field is frozen from the app side, only `BackfillService` hydration/diagnostic and the `BootstrapRoot` DEBUG logger continue to read it through 9-E; commit `5819fa6`). Sub-slices are independently revertible. Recommended ship spacing: **9-pre tests** ✅ → **9-pre diagnostic** ✅ → **9-A1** ✅ → **9-A2** ✅ → **9-A.5 audit decisions** ✅ → **9-B1** ✅ → **9-B2** ✅ → **9-C1** ✅ (orphan-fallback rewrite shipped 2026-05-22) → **9-C2** ✅ (`resolvedTemplates` Tier 3 strip shipped 2026-05-22 — original TestFlight-diagnostic gate bypassed by maintainer decision; bootstrap hydration deterministically covers the same population) → **9-D** ✅ (editor UI removal shipped 2026-05-22) → **9-E pre-flight migration decision** (gated on the diagnostic's `defaultTemplatesWithTargetWeight` + `defaultTemplatesNonWorkingKind` counters from real-user data) → **9-E field deletion + orphan `SetTemplate` sweep + diagnostic logger removal**.
