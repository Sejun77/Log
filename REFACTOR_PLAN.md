...
**9-B — Eliminate non-Tier-3 reads of `defaultTemplates` (split into 9-B1 routine-editor reads / 9-B2 ActiveWorkout swap path; the audit recommended the split to keep each commit reviewable):**

**9-B1 — Routine-editor reads:**
...
- [ ] Add `LogTests/SupersetCountGateTests.swift` pinning the prescription-driven count behavior across `appendBlock` + `SupersetPicker.setCount` paths. If `normalizeRoutineModel` is kept rather than removed, also pin its new comparison basis
- [ ] **Ship 9-B1 standalone.** All three reads have clean post-9-A replacements; risk is low because every legacy slot already has `prescription?.sets != nil` after 9-A2 hydration. No `Exercise.defaultTemplates` read is removed beyond these three call sites — Tier 3 stays intact for `resolvedTemplates` until 9-C
...
Phase 9 is **not closed** — 9-B1 / 9-B2 / 9-C / 9-D / 9-E (implementation) are still pending. Shipped so far: pre-9 cleanup (8-C), 9-pre regression tests, **9-A** in full (9-A1 hydration service + 9-A2 bootstrap wiring), and **9-A.5 audit decisions** (planning). Sub-slices are independently revertible. Recommended ship spacing: **9-pre tests** ✅ → **9-pre diagnostic** (TestFlight; recommended before 9-C, REQUIRED before 9-E) → **9-A1** ✅ → **9-A2** ✅ → **9-A.5 audit decisions** ✅ → **9-B1** (routine-editor reads) → **9-B2** (ActiveWorkout swap path) → release gap + observe diagnostic → **9-C** → **9-D** → **9-E pre-flight migration decision** (gated on diagnostic counters c/d) → **9-E field deletion**.
