...
**9-C — Remove Tier 3 from `resolvedTemplates` + resume fallback (split per 2026-05-21 planning audit into 9-C1 orphan fallback + 9-C2 `resolvedTemplates` strip; ship 9-C1 first):**

**9-C1 — `WorkoutResumeService` orphan-fallback rewrite (recommended first):**

- [ ] Rewrite `WorkoutResumeService.swift:254-268` orphan-fallback branch (item has no `setLogs` AND no `plannedPrescriptionSnapshot`): no longer read `ex.defaultTemplates`. Per 9-A.5 audit decision, do NOT carry `kind != .working` rows or `targetWeight` — both losses are recorded as accepted. Per the 2026-05-21 audit, **prefer routing through a `makeSwapDefaultTemplates(...)`-style helper** (the same pattern 9-B2's follow-up applied to `planFromRoutine`'s swap-reconciliation branch): produces N uniform `.working` rows where N comes from `AppSettings.defaultSets` (no slot/snapshot hints exist in this branch), with `targetWeight: nil`, rest from `AppSettings.defaultRestBetweenSets`, and duration (time-based only) from the hardcoded 60s 9-A1 fallback. Gives the orphan-resume user a usable baseline to log against instead of an empty plan. If `makeSwapDefaultTemplates` can't be called directly from a service (e.g. due to View/helper isolation), extract an equivalent service-safe helper alongside `BackfillService.hydrate(_:from:)` or add a small free function in a shared module
- [ ] Add `LogTests/WorkoutResumeServiceTests` cases:
  - `testFallbackUsesAppSettingsWhenNoLogsAndNoSnapshotAndNoDefaults` — empty defaults → AppSettings-derived templates, all `.working`, `targetWeight: nil`
  - `testFallbackDoesNotReadExerciseDefaultTemplatesWhenSnapshotMissing` — fixture has `defaultTemplates = [working(reps: 99, weight: 999, kind: .dropset)]`; assert NO sentinel value (99, 999, `.dropset`) surfaces in `finalTemplates`
  - `testFallbackPreservesEmptyBlockSkipWhenExerciseIsNil` — re-pin existing `guard let ex = item.exercise else { return nil }` (anchors it during the rewrite)
- [ ] Invert `SlotPrescriptionResolutionTests.testWorkoutResumeFallsBackToExerciseDefaultsWhenNoLogsAndNoSnapshot` (cross-test from the 9-pre slice) to expect AppSettings-derived templates instead of Exercise defaults. Rename to reflect the new contract
- [ ] **Independent of diagnostic observation** — 9-A's hydration does NOT cover this branch (operates on `WorkoutItem`, not `RoutineExercise`), and the orphan-resume population is independent of the slot-hydration counters. Can ship as soon as 9-B2 is on a release the team is comfortable with
- [ ] **Ship 9-C1 standalone.** Net diff: ~25 LOC production + ~80 LOC test (2-3 new cases + 1 invert/rename). After 9-C1, `grep -rn "defaultTemplates" Log/Services/` should show only the diagnostic helper's read in `BackfillService.diagnoseDefaultTemplatesRisk(in:)`
...
Phase 9 is **not closed** — 9-C1 / 9-C2 / 9-D / 9-E are still pending. Shipped so far: pre-9 cleanup (8-C), 9-pre regression tests, **9-A** in full (9-A1 hydration service + 9-A2 bootstrap wiring), **9-A.5 audit decisions** (planning), **9-B** in full (9-B1 routine-editor reads + 9-B2 ActiveWorkout swap path incl. follow-up swap/resume/timer fixes), the **9-pre / pre-9-E `defaultTemplates` risk diagnostic** (DEBUG-only launch logging + test-callable helper), and **9-C planning audit** (split 9-C into 9-C1 + 9-C2). Sub-slices are independently revertible. Recommended ship spacing: **9-pre tests** ✅ → **9-pre diagnostic** ✅ → **9-A1** ✅ → **9-A2** ✅ → **9-A.5 audit decisions** ✅ → **9-B1** ✅ → **9-B2** ✅ → **9-C1** (orphan-fallback rewrite — independent of diagnostic observation) → release gap + observe diagnostic on TestFlight → **9-C2** (`resolvedTemplates` Tier 3 strip — gated on `residualEmptyContentSlots == 0 AND slotsNeedingTier3 == 0`) → **9-D** → **9-E pre-flight migration decision** (gated on `defaultTemplatesWithTargetWeight` + `defaultTemplatesNonWorkingKind`) → **9-E field deletion**.
