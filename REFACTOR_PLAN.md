...
**Pending (5.2 — superset round gating when manually switching exercises):**

- [ ] During an active superset, the user can manually navigate back to the first exercise after partially completing a round and log a second set of that exercise before every other exercise in the round has logged its first set. The next round should stay **locked** until every exercise in the round is `isWorkingSetComplete` — parent logged AND, for sets where a dropset technique applies, all configured drops logged — i.e. the same completeness definition the rest planner already enforces in `RestPlanner.restSecondsAfterSupersetRound(_:)` (Phase 7.4-C.2). The rest planner already returns `nil` while the round is incomplete (mid-round suppression), so the bug is purely on the logging-gate side, not the rest-decision side. Likely fix: gate `canLog` / set-row enablement in `ActiveWorkoutView.buildSetRow` so set index `i + 1` of any exercise in a superset block is locked until `supersetRoundComplete(block:setIndex: i)` is true. May also need to refresh focus-advance semantics so manual back-navigation doesn't bypass the gate. Belongs in Phase 5.2 because it's the "superset flow streamline" half of that slice; do NOT mark 5.2 complete until this lands
...
- [x] **Persistence formats intentionally preserved** (Slice B will handle): `ParentDraftStore.persist/clear/load(slotID:)` still receives `Exercise.id` from every callsite; drop-key strings remain `"\(exerciseID)_\(setIndex)_\(sub)"` with `exerciseID = exercise.id` in `rehydrate`, `undoSetLog`, and `buildDropSection`; `DropWeightDraftStore` key format and on-disk layout unchanged. In-flight drafts written by Slice-A-or-older builds remain readable; nothing on disk changes this slice
...
**Pending (5.2 — active-workout identity Slice B: persisted draft-store keys → `routineSlotID`):**

- [ ] Decide and ship migration of `ParentDraftStore` and `DropWeightDraftStore` on-disk key formats from `Exercise.id` to `routineSlotID`. Today (post-Slice-A): in-memory state is correctly per-slot, but the persistence layer alone still collides for duplicate Exercise across slots — both slots write/read the same draft entry. A force-quit + cold-resume of a session with typed-but-unlogged drafts in BOTH slots will resurrect the same draft into both slots. Fix needs: (a) update every `parentDraftStore?.persist/clear/load` caller to pass `exercise.routineSlotID` (sites currently using `exercise.id` or the `exerciseID` local in `undoSetLog` / `inputBindings` / `durationBinding` / `buildSetRow.onLog`); (b) update drop-key string construction in `rehydrate`, `undoSetLog`, `buildDropSection` to `"\(routineSlotID)_\(setIndex)_\(sub)"`; (c) decide migration strategy for in-flight drafts — options are (i) one-time eviction at version bump (recommended; drafts are short-lived per-workout), (ii) load-time rewrite of `"<exerciseID>_..."` keys via the active plan's `exerciseID → routineSlotID` map (only works while the originating plan is still loadable), or (iii) lossy-on-update with a release note. Add tests that pin the new on-disk key format in both stores; pre-existing `testStorageKeyFormatIsStable` tests will need to update or be replaced with version-aware variants

**Pending (5.2 — duplicate-`Exercise` inside superset verification):**

- [ ] Slice A's correctness for duplicate Exercise was verified manually for two slots in **separate non-superset blocks**. The same-Exercise-twice-inside-one-superset variant could not be exercised because the routine editor has no UI path to **add** an exercise to an existing superset block (drag-to-reorder and remove exist; insert does not). Blocked until: (a) a routine-editor flow lands that supports inserting into an existing superset, OR (b) a seed-data path (test fixture / debug-only build flag) constructs such a routine. Once unblocked, verify: log slot A's set 1 inside the superset → slot B's set 1 remains gated as "next-in-round-incomplete", round rest does NOT fire (the rest planner's mid-round suppression already reads from the now-slot-keyed `isWorkingSetComplete`, so the unit-test surface is already covered — only the integration / UI verification remains)

**Pending (5.2 — duplicate-`Exercise` notes apply-back ambiguity):**

- [ ] In-memory `activeGuard.notesCache` is now per-slot (Slice A). However, the apply-back path `persistExerciseNotesOnlyForCurrentExercises` still writes onto the library `Exercise.notes` via `fetchExercise(by: planEx.currentExerciseID)`. When the same Exercise occupies two slots with different cached notes, iteration walks both slots and assigns `ex.notes = text` for the shared `Exercise` — the second slot's notes overwrite the first ("last-slot-wins"). Pre-Slice-A this collision happened in-memory too (both slots shared the cache entry); Slice A fixed the in-session display but not the apply-back semantics. **Product decision needed** — possible directions: (a) keep current "last-slot-wins" behavior, document explicitly; (b) only the "primary" slot (e.g., lowest `RoutineExercise.order`) writes back to `Exercise.notes`, secondary slots keep their notes session-only via `WorkoutItem.templateNotesSnapshot`; (c) stop writing to library `Exercise.notes` from the workout entirely and persist all per-slot notes in `WorkoutItem.templateNotesSnapshot` / `RoutineExercise.templateNotes`. Out of scope for this codebase slice; reopen when product direction is chosen
