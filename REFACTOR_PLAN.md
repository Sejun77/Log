...
Phase 9 is **not closed** — 9-D / 9-E are still pending (Phase 9-C is complete: 9-C1 + 9-C2 both shipped 2026-05-22). Shipped so far: pre-9 cleanup (8-C), 9-pre regression tests, **9-A** in full (9-A1 hydration service + 9-A2 bootstrap wiring), **9-A.5 audit decisions** (planning), **9-B** in full (9-B1 routine-editor reads + 9-B2 ActiveWorkout swap path incl. follow-up swap/resume/timer fixes), the **9-pre / pre-9-E `defaultTemplates` risk diagnostic** (DEBUG-only launch logging + test-callable helper), **9-C planning audit** (split 9-C into 9-C1 + 9-C2), and **9-C** in full (**9-C1** `WorkoutResumeService` orphan-fallback rewrite — `defaultTemplates` read removed from `Log/Services/WorkoutResumeService.swift`; orphan-resume now uses `makeSwapDefaultTemplates(...)` at AppSettings defaults; and **9-C2** `resolvedTemplates` Tier 3 strip — both no-context `Entities.swift` and context-aware `RoutineExercise+Helpers.swift` resolvers are now strictly 2-tier (Tier 1 setTemplates → Tier 2 prescription → `[]`), commit `e9ba172`). Sub-slices are independently revertible. Recommended ship spacing: **9-pre tests** ✅ → **9-pre diagnostic** ✅ → **9-A1** ✅ → **9-A2** ✅ → **9-A.5 audit decisions** ✅ → **9-B1** ✅ → **9-B2** ✅ → **9-C1** ✅ (orphan-fallback rewrite shipped 2026-05-22) → **9-C2** ✅ (`resolvedTemplates` Tier 3 strip shipped 2026-05-22 — original TestFlight-diagnostic gate bypassed by maintainer decision; bootstrap hydration deterministically covers the same population) → **9-D** → **9-E pre-flight migration decision** (gated on `defaultTemplatesWithTargetWeight` + `defaultTemplatesNonWorkingKind`) → **9-E field deletion**.
