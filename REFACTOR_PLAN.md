...
- [ ] **Cold-resume fallback rewrite must be deliberate** (per 9-A.5 audit): when updating `WorkoutResumeService.swift:218-232`, seed templates from `PlannedPrescriptionSnapshot` only — do NOT carry `kind != .working` rows from `Exercise.defaultTemplates`. The audit recorded this loss as accepted; the test for this branch should pin that warmup/dropset rows are absent from the resumed plan so the contract is enforced. **Note:** 9-B2's follow-up fix already rewrote `planFromRoutine`'s **swap-reconciliation** branch to route through `makeSwapDefaultTemplates(...)` (uniform `.working` rows, no warmup/dropset carry), so the same pattern can be applied to the `planFromWorkoutItems` orphan fallback when 9-C ships
...
Phase 9 is **not closed** — 9-C / 9-D / 9-E are still pending. Shipped so far: pre-9 cleanup (8-C), 9-pre regression tests, **9-A** in full (9-A1 hydration service + 9-A2 bootstrap wiring), **9-A.5 audit decisions** (planning), **9-B** in full (9-B1 routine-editor reads + 9-B2 ActiveWorkout swap path incl. follow-up swap/resume/timer fixes), and the **9-pre / pre-9-E `defaultTemplates` risk diagnostic** (DEBUG-only launch logging + test-callable helper). Sub-slices are independently revertible. Recommended ship spacing: **9-pre tests** ✅ → **9-pre diagnostic** ✅ → **9-A1** ✅ → **9-A2** ✅ → **9-A.5 audit decisions** ✅ → **9-B1** ✅ → **9-B2** ✅ → release gap + observe diagnostic on TestFlight → **9-C** → **9-D** → **9-E pre-flight migration decision** (gated on `defaultTemplatesWithTargetWeight` + `defaultTemplatesNonWorkingKind`) → **9-E field deletion**.
