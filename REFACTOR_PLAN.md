...
**Pending (7.4-C.2 — superset rest branch extraction + tests):**

- [ ] Extract the `block.isSuperset` round-rest logic out of `ActiveWorkoutView.restSecondsAfterCurrentLog` into `RestPlanner` (likely a second pure entry-point such as `RestPlanner.restSecondsAfterSupersetRound(_:)` plus a `SupersetRestContext`, rather than overloading `RestContext`). Coverage to pin: round-completion gating (wait until every exercise in the round is "fully complete" — parent log AND every configured drop), base round rest from `block.supersetRoundRestSeconds`, after-dropset round rest using `plannedRestBetweenSets ?? priorWorkingRest(in:upTo:)` with max-combine across exercises, normal-round rest using `plannedRestBetweenSets ?? template.restSecondsAfter` with max-combine, next-round-has-dropset skip via `lastRoundIndex(in:)`, and the superset transition-rest replacement (`block.restAfterSeconds` replaces — does NOT add to — the round rest, gated on `supersetRoundComplete`). `isWorkingSetComplete` / `supersetRoundComplete` / `lastRoundIndex` / `priorWorkingRest` will need to either move with the planner or be passed in as closures
