...
Last updated: 2026-05-18 (KST) — Phase 7 Slices 7.0 + 7.1 + 7.2 + 7.3 + 7.4 (RestTimer slice) + 7.5 complete: `LogTests` XCTest target wired with an in-memory `ModelContainer` harness and full suite at 38/38; the 7.4 sub-slice added 7 pure-XCTest cases pinning `RestTimer.stableNotificationID(workoutID:slotID:)` for determinism, distinct-input separation, non-empty output, the `"rest."` prefix, and the both-UUIDs-present format that notification cancellation depends on — no production code touched. `ParentDraftStore` extraction + tests and the rest-decision extraction remain pending under 7.4. Slice 7.3 extracted Phase 6.B backfill into `BackfillService`; Slice 7.2 extracted `RoutineLabelResolver`; Slice 7.5 aligned the test target's deployment to 18.5 and documented the concrete-simulator + app-hosted policy in CLAUDE.md. Host-less conversion attempted and reverted (iOS app targets can't link `@testable` symbols without `TEST_HOST` / `BUNDLE_LOADER`). Phase 6.B Slice C.1 / Slice B remain shipped; Slice C.2 grouping and rename verification still pending
...
**Pending (7.4 — `ParentDraftStore` extraction + tests):**
...
**Pending (7.4 — rest-decision extraction):**
