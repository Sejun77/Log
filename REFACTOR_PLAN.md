...
**Pending (11.6-B — `SessionPlanResolver` service, low risk; mirrors `RestPlanner`):**

- [ ] Create `Log/Services/SessionPlanResolver.swift` (~100 LOC) holding pure static functions that take `sessionPlans[slotID]` + the prescription `snapshot` + `resolvedTemplates` as parameters:
  - `SessionPlanResolver.effectiveSetCount(sessionPlan:snapshot:resolvedTemplates:) -> Int`
  - `SessionPlanResolver.plannedRepTarget(sessionPlan:template:) -> Int`
  - `SessionPlanResolver.plannedDurationTarget(sessionPlan:template:) -> Int?`
...
- [ ] Same pattern as `RestPlanner` (Phase 7.4-C): caller passes state in; service is a pure namespace; no `ActiveWorkoutView` member access required. **Zero access bumps.**
- [ ] Add `SessionPlanResolverTests.swift` (~15–20 pure-XCTest cases) pinning the precedence chain (`sessionPlan` > `snapshot` > `template count` fallback); each of the 5 helpers gets a covering test. Brings total to ~130–135 tests
- [ ] Update ~20 call sites inside `ActiveWorkoutView.swift` from `effectiveSetCount(for: ex, …)` to `SessionPlanResolver.effectiveSetCount(sessionPlan: sessionPlans[ex.routineSlotID], …)` etc. Optionally keep thin private wrappers on `ActiveWorkoutView` if call-site brevity matters
- [ ] Expected `ActiveWorkoutView.swift` reduction: 3,027 (post-11.6-A baseline) → ~2,950 (−~80 LOC). New service + tests ~200 LOC
- [ ] Manual smoke: open Edit Plan → change `sets` / `repMin` / `repMax` / `restBetweenSets` / `restAfterExercise` → verify body recomputes and the rest decision after the next log uses the new values

**Pending (11.6-C — per-concern extension files, medium risk; explicit access-bump trade-off; ship as separate sub-slices):**

- [ ] Move clustered private helpers onto `extension ActiveWorkoutView` declarations in dedicated files. Ship one cluster per sub-slice; the order below is recommended (least surface-widening first):
...
- [ ] **Required access-control trade-off**: Swift's `private` is invisible from extensions in other files. Each sub-slice must bump exactly the `@State`-or-helper members it reads/writes from `private` to default-internal. After 11.6-C ships in full, the bumps likely include `currentBlockIndex`, `currentExerciseIndex`, `loggedByExercise`, `dropsLoggedByExercise`, `dropWeightInput`, `dropRepsInput`, `dropWeightUserEdited`, `sessionPlans`, `itemsByExerciseID`, `inputsByExerciseID`, `workout`, `plan`, `preEditRepStrs`, `preEditDurStrs`, `rest`, `setTimer`, `showRestOverlay`, plus the helpers `isWorkingSetComplete`, `currentBlock`, `currentExercise`, `fetchWorkout`, `fetchExercise`. Each sub-slice must enumerate the bumps it actually performs and add a top-of-file MARK in `ActiveWorkoutView.swift` listing them — surface widening is a real cost
- [ ] **No `@ViewBuilder` methods move under 11.6-C.** The four `@State`-capturing `@ViewBuilder`s (`buildSetRow`, `buildWarmupRow`, `buildDropSection`, `buildWorkingSetGroup`) plus `planSummarySection` and `buildTechniqueChips` stay in `ActiveWorkoutView.swift` and remain Phase-12 (viewmodel hoist) candidates
- [ ] Expected `ActiveWorkoutView.swift` reduction after full 11.6-C: ~2,950 (post-11.6-B baseline) → ~2,150–2,300. The body itself (~475 LOC) + the six `@ViewBuilder` methods (~1,014 LOC combined) + struct properties (~64 LOC) form the structural floor; further reduction is Phase 12
- [ ] Manual smoke per sub-slice (shape to the cluster — persistence: force-quit / cold-resume / rest-notification flow; swap: normal swap + locked-routine swap + dirty-plan swap; snapshot: cold-resume from a routine that no longer exists; logging: parent log + drop log + undo path for both; superset helpers: 3-exercise superset with a dropset on round 2; techniques: per-set technique chips + multi-index applies-to)

**Pending (11.6-D — `restSecondsAfterCurrentLog` extraction, optional follow-up to 11.6-C):**

- [ ] Only after 11.6-B and 11.6-C ship in full: `restSecondsAfterCurrentLog(setIndex:template:block:exercise:)` is already a thin shell over `RestPlanner.*` calls. Its body reads `currentBlockIndex`, `plan`, and calls `effectiveSetCount` (→ `SessionPlanResolver` after 11.6-B), `plannedRest*` (same), `lastRoundIndex` (→ `+SupersetHelpers` after 11.6-C), `dropsetTechniqueApplying` (→ `+Techniques` after 11.6-C). Becomes a candidate for `Log/Main/ActiveWorkout/ActiveWorkoutView+RestDecisions.swift` once those dependencies are reachable from another file
- [ ] Expected reduction: ~−160 LOC if extracted; behavior-preserving since the planner already owns the decision tree. Optional — skip if 11.6-C did not ship

**Phase 11 closure note**: even with 11.6-A complete, Phase 11 remains open. 11.6-B is a clean RestPlanner-style service extraction with no access-bump cost; 11.6-C is the larger move that forces the documented `@State` surface widening. The plan should be considered "functionally complete for the file-decomposition objective" once 11.6-B ships; 11.6-C is **optional** and can be explicitly deferred to Phase 12 (since the bulk of remaining `ActiveWorkoutView.swift` LOC after 11.6-A+B will be the body + `@ViewBuilder` methods, which are Phase-12 work regardless).
...
- `restSecondsAfterCurrentLog(setIndex:template:block:exercise:)` — post-Phase-7.4-C, this method is largely a shell over `RestPlanner` calls plus a few inline guards. Now tracked as **Phase 11.6-D** (optional follow-up to 11.6-C) above. Reachable from another file once 11.6-B has lifted `effectiveSetCount` / `plannedRest*` into `SessionPlanResolver` and 11.6-C has moved `lastRoundIndex` / `dropsetTechniqueApplying` to extension files

**Phase-11 progress snapshot:**

| Target file | Pre-11.1 | Post-11.1 | After 11.2 | After 11.3 | After 11.4 | After 11.5 | After 11.6 |
|---|---:|---:|---:|---:|---:|---:|---:|
| `ActiveWorkoutView.swift` | 3,849 | **3,695** | 3,695 | 3,695 | **3,061** | 3,061 | ~2,150–2,300 † |
...
† Post-11.6-A intermediate: **`ActiveWorkoutView.swift` 3,061 → 3,027 LOC (−34)**. The "After 11.6" column shows the projected floor only if 11.6-B + 11.6-C both ship in full; 11.6-A alone moves the number by ~30 LOC. If 11.6-C is deferred to Phase 12 (a valid option — see "Phase 11 closure note" above), the realistic post-11.6-B floor is **~2,950**.
